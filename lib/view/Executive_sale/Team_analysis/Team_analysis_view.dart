...
class Team_analysis extends StatefulWidget {
  ...
  @override
  void dispose() {
   salesViewModel.erp_Employee_controller.dispose();
    super.dispose();
  }
  void showRequestPopup(BuildContext context) {
      ...
      builder: (BuildContext context) {
        return AlertDialog(
          ...
          actions: [
            ...
            ElevatedButton(
              onPressed: () {
                String requestText = _requestController.text;
                if (requestText.isNotEmpty) {
                  // Here you can handle the request (e.g., send to server, store it, etc.)
                  print('Request Added: $requestText');
                }
                Navigator.of(context).pop();
              },
              child: Text('Submit'),
                ...
                if (user.designation == "SUP") {
                 ...
                 // getdsftarget(user.empCode, formattedStartDate,formattedEndDate);
                  ...
                  setState(() {
                    ...
                    user.reports = LocalDatabase.select(user.empCode, '0');
  ...
  void showfitler() async {
    ...
    await LocalDatabase.fetch_companies();
    await LocalDatabase.fetch_branch();
    setState(() {
      try {
  ...
  Future<void> load( String code ) async {
    ...
    employeeList = await LocalDatabase.fetch_heirarchy();
  ...
  Future<void> branchcompany() async {
    await LocalDatabase.initializeDatabase();
    final data = await LocalDatabase.fetch_branch();
  ...
  Future<void> teamcompany() async {
    ...
    await LocalDatabase.initializeDatabase();
    final data = await LocalDatabase.fetch_companies();
  ...
  void fetchheirarchy() async {
    await LocalDatabase.initializeDatabase();
    originalList = await LocalDatabase.heirarchy_for_search(); // store full list
    setState(() {
              ...
              actions: [
                ...
                InkWell(
                    onTap: () async {
                     ...
                     salesViewModel.attendancesheet(context,salesid);
                    },
                    child: Image.asset("assets/attendance.png", color: Colors.white, height: 28,width: 28,)),
                ...
                onTap: ()  {
                  ...
                  return
                    InkWell(
                      onTap: (){
                        setState(() {
                          name = data.empName.split("||")[0];
                          salesid =data.empCode;
                         rootlist.clear();
                          rootlist.add(
                            UserDetails(
                          ...
                          data.reports = LocalDatabase.select(data.empCode, '0');
                        });
                      },
                      child: Container(
                                                ...
                                                child:  InkWell(
                                                  onTap: () async {
                                                    ...
                                                    if(datalist.isCheck ==true)
                                                    {
                                                      selectedIndex =index;
                                                      await LocalDatabase.initializeDatabase();
                                                      datalist.reports =  LocalDatabase.select_through_depth(datalist.Depth.toString());
                                                    }
                                                    else {
                                                      //depthList.clear();
                                                    }
                                        ...
                                        return
                                          HeirarchyCard(
                                              ...
                                              onTap: () async
                                              {
                                                ...
                                                if (datas['EmpDesignation'] != "DSF") {
                                                  name=datas['EmpName'].split("||")[0];
                                                  salesid=datas['EmpCode'];
                                              ...
                                              attendanceontap: (){
                                               // salesViewModel.dsfuserattendance(datas['EmpCode']);
                                                salesViewModel.dsfattendancesheet(context, datas['EmpCode']);
                                              },
