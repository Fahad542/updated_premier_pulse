...
// class get_branches_Repository {
//   Future<List<get_bracnhes_model>> fetchData(
...
//         return List<get_bracnhes_model>.from(data.map((json) => get_bracnhes_model.fromJson(json)));
//       } else {
...
class get_branches_Repository {
  ...
  Future<List<get_bracnhes_model>> fetchData( String startdate, String enddate, List<int>companyid) async {
    final url = 'https://api.psplbi.com/api/brmeasure';
    print(url);
    final requestData =
    [
      {
...
print(json.encode(requestData));
    ...
    List<get_bracnhes_model>.from(
        data.map((json) => get_bracnhes_model.fromJson(json))));
  }
}
